...
module.exports = function (logger, config, knexClient) {
  ...
  var fileHelper = require('../utils/file')(config);
  ...
  router.route('/:id')
    ...
    .delete(function(req, res, next) {
      knexClient('circlrs').where({ id: req.circlr.id, user_id: req.user.id }).del()
        .then(function() {
          return knexClient('circles').where({circlr_id: req.circlr.id}).del();
        })
        .then(function() {
          return knexClient('circles_photos').where({circlr_id: req.circlr.id}).del();
        })
        .then(function() {
          return knexClient('photos').where({circlr_id: req.circlr.id}).returning('path').del();
        })
        .then(function(result) {
          // path = XXX/YYY.extension
          var paths = R.map(function(path) {
            return path.indexOf('/') > -1 ? path.substr(0, path.indexOf('/')) : path;
          }, result);

          return fileHelper.deleteFolders(paths);
        })
        .then(function() {
