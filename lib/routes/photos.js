...
module.exports = function (logger, config, knexClient) {
  ...
  router.route('/:id')
    ...
    .delete(function(req, res) {
      knexClient.select('id', 'path').from('photos').where({id: req.params.id, circlr_id: req.circlr.id }).limit(1)
        .then(function(row) {
          if (!row || row.length === 0) {
            return next(new errors.EntityNotFoundError('photos'));
          }

          var fullSubDir = row[0].path;
          var subDir = fullSubDir.indexOf('/') > -1 ? fullSubDir.substr(0, fullSubDir.indexOf('/')) : fullSubDir;

          return fileHelper.deleteFolders([subDir]);
        })
        .then(function() {
          return knexClient('photos').where({id: req.params.id, circlr_id: req.circlr.id }).del();
        })
        .then(function () {
          return knexClient('circles_photos').where({ circlr_id: req.circlr.id, photo_id: req.params.id }).del();
        })
        .then(function () {
